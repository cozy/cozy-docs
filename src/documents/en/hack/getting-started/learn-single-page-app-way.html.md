...
### What you will achieve
* Learning the basics of [Angular.js](https://angularjs.org/), a MVC JavaScript framework

### Source code
The source code of this tutorial can be found [here](https://github.com/lemelon/cozy-tutorial-spa-angular-final).
...
AngularJS is an Open-Source framework maintained by Google and is a truly amazing approach to developing JS-heavy web applications. As such, the core team is constantly growing, resulting in innovation and tools that improve developer productivity.
It is also the 3rd most-starred project on GitHub and is having more questions on StackOverflow than Ember and Backbone combined.
As a result, your application will be made of:
...
```javascript
// app/assets/scripts/books/Home.Ctr.js
function add(bookItem) {
    ...
    book.addBook().then(function() {
        // vm.books will be binded to home.html with ng-repeat. 
...
```bash
<tr ng-repeat="book in home.books track by $index">
    <td>{{book.title}}</td>
    <td>{{book.link}}</td>
    <td>
        <button class="btn btn-default" ng-click="home.removeBook($index, book.id)" type="submit">Delete</button>
    </td>
</tr>
```
...
```javascript
// app/assets/scripts/books/Book.Fctr.js

// ! important : define your route depended to the name of your app
function addBook() {
    var self = this;
    // will call the post route of the book API
...
### Removing a bookmark
In the bookmark controller (Home.Ctrl.js), add the "removeBook" feature like we did earlier for the "add" one.

```javascript
// app/assets/scripts/books/Home.Ctrl.js
function removeBook(index, bookId) {
    // set the book id to delete
    book.setId(bookId);

    // delete the book related to the book id
...
function deleteBook() {
    var self = this;
    // will call the get route of the book API
